...
async function buildGpaPdf(record: any): Promise<Uint8Array> {
  ...
  txt(page, `Name: ${record.studentName || "ANONYMOUS"}`,  50, 162, 10, reg);
  txt(page, `Register No: ${record.registerNo || "N/A"}`, 300, 162, 10, reg);
  txt(page, `Semester: ${record.semester || 1}  |  Regulation: ${record.regulation || "R2021"}  |  Dept: ${record.department}`, 50, 176, 9, reg);

  const tableTop = 210;

  // Subject-level table headers
  ...
  let totalCredits = 0;
  const subjectsToRender = (record.subjects && record.subjects.length > 0)
    ? record.subjects.filter((s: any) => s.grade && String(s.grade).trim() !== '')
    : [];

  subjectsToRender.forEach((s: any, idx: number) => {
    ...
    txt(page, String(credits), TCOLS[3] + 3, y + 7, 9, reg, C.dark, TWIDTHS[3]);
    txt(page, String(s.grade || "N/A"), TCOLS[4] + 3, y + 7, 9, bold, C.dark, TWIDTHS[4]);
    y += ROW_H;
  });

  // Totals bar
  fillRect(page, 40, y, PW - 80, 22, C.purple);
  txt(page, `Total Credits: ${record.totalCredits || totalCredits}`, 50, y + 6, 10, bold, C.navy);
  y += 22;
