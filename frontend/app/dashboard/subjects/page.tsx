          ...
          <div className="bg-[#071830] border border-sky-500/20 max-w-md w-full rounded-2xl p-6 relative z-10 shadow-2xl animate-scale-in flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col min-h-0 flex-1">
              <div className="flex-1 overflow-y-auto pr-1.5 space-y-4 scrollbar-thin">
                <div>
                  <label className="block text-[10px] font-bold text-sky-300 uppercase tracking-wider mb-2">Subject Code *</label>
                  <input type="text" required value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="e.g. CS3401" disabled={!canEditSubjects} className="w-full bg-[#071830] border border-sky-500/20 focus:border-sky-500 rounded-xl px-4 py-2.5 text-xs text-white placeholder-sky-300/20 focus:outline-none transition-all disabled:opacity-40" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-sky-300 uppercase tracking-wider mb-2">Subject Name *</label>
                  <input type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Database Management Systems" disabled={!canEditSubjects} className="w-full bg-[#071830] border border-sky-500/20 focus:border-sky-500 rounded-xl px-4 py-2.5 text-xs text-white placeholder-sky-300/20 focus:outline-none transition-all disabled:opacity-60" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-sky-300 uppercase tracking-wider mb-2">Credits *</label>
                    <input type="number" required min="1" max="10" value={credits} onChange={e => setCredits(Number(e.target.value))} disabled={!canEditSubjects} className="w-full bg-[#071830] border border-sky-500/20 focus:border-sky-500 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none transition-all disabled:opacity-60" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-sky-300 uppercase tracking-wider mb-2">Semester</label>
                    <select value={semester} onChange={e => setSemester(Number(e.target.value))} disabled={!canEditSubjects} className="w-full bg-[#071830] border border-sky-500/20 focus:border-sky-500 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none disabled:opacity-60">
                      {[...Array(8)].map((_, i) => <option key={i+1} value={i+1}>Semester {i+1}</option>)}
                    </select>
                  </div>
                </div>
                <div>
                  ...
                  <select value={regulation} onChange={e => setRegulation(e.target.value)} disabled={!canEditSubjects} className="w-full bg-[#071830] border border-sky-500/20 focus:border-sky-500 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none disabled:opacity-60">
                    {(() => {
                      const opts = [...regulations];
                ...
                <div>
                  ...
                  <select value={department} disabled={currentUser?.role === 'dept_admin' || currentUser?.role === 'staff' || !canEditSubjects} onChange={e => setDepartment(e.target.value)} className="w-full bg-[#071830] border border-sky-500/20 focus:border-sky-500 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none disabled:opacity-50">
                    {departments
                      .filter(d => currentUser?.role === 'super_admin' || d.code === currentUser?.department)
                      .map(d => <option key={d._id} value={d.code}>{d.name}</option>)}
                  </select>
              ...
              <div className="flex justify-end gap-3 pt-4 mt-4 border-t border-sky-500/10 shrink-0">
                ...
                <button type="submit" disabled={submitting || !canEditSubjects} className="px-5 py-2.5 bg-sky-500 hover:bg-indigo-550 text-xs text-white font-bold rounded-xl transition-all shadow-md disabled:opacity-50 cursor-pointer">
