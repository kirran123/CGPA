...
* **Fix**: Added dynamic `visible` / `invisible` (i.e. `visibility: visible` / `visibility: hidden`) utility toggles to the outer drawer container in [layout.tsx](file:///c:/Users/kishore%20ST/Desktop/CGPA/frontend/app/dashboard/layout.tsx). When `sidebarOpen === false`, the entire drawer is marked as `invisible`, forcing the browser to completely ignore it during hit-testing and allowing all scroll gestures to pass through instantly. The transition classes ensure it fades smoothly when opened/closed. Also added `pointer-events: none !important;` and `z-index: 0;` to background `.orb` classes in [globals.css](file:///c:/Users/kishore%20ST/Desktop/CGPA/frontend/app/globals.css) to prevent compositing layers from capturing events.

### 15. Responsive Modal Max-Height & Screen Fitting
* **Issue**: The Staff and Department modals had their maximum height set to `calc(100dvh - 0px)` directly via inline style. On desktop screens, because the parent wrapper had padding (`p-0 sm:p-4`), this caused the modal to overflow the padded wrapper and get clipped at the top and bottom, hiding/cutting off the submit ("Save Member") and "Cancel" buttons.
* **Fix**: Replaced the inline styles with responsive Tailwind classes (`max-h-[100dvh] sm:max-h-[calc(100dvh-3rem)]`) on the modal containers in [staff/page.tsx](file:///c:/Users/kishore%20ST/Desktop/CGPA/frontend/app/dashboard/staff/page.tsx) and [departments/page.tsx](file:///c:/Users/kishore%20ST/Desktop/CGPA/frontend/app/dashboard/departments/page.tsx). This keeps them as full-screen bottom-sheets on mobile, but limits their height on desktop to leave breathing room inside the padded wrapper. Since the form content remains scrollable, the action buttons are kept sticky and 100% visible on all desktop resolutions.
